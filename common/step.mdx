...
<Steps>
  ...
  <Step title="Preprocessing">
    ...
    ```
    astrape preprocess /path_to_survey_folder
    ```
  ...
  <Step title="Run Inversion">
    Astrape has two methods for running an inversion, 2.5D and 3D. Run a 2.5D inversion with the command:
    ```
    astrape 2d run /path_to_survey_folder
    ```
    and run the inversion area in full 3D, use the following command:
    ```
    astrape 3d run /path_to_survey_folder
    ```
  ...
  <Step title="Postprocessing">
    ...
    ```
    astrape postprocess /path_to_experiment_folder
    ```
